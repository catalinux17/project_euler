...
def get_sum_diag(i: int, j: int) -> int:
    return matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2] * matrix[i + 3][j + 3]

def get_sum_diag_inverse(i: int, j: int) -> int:
    return matrix[i][j] * matrix[i - 1][j - 1] * matrix[i - 2][j - 2] * matrix[i - 3][j - 3]

def get_sum_down(i: int, j: int) -> int:
...
for i in range(17):
    for j in range(20):
        print(i, j)
        sum = get_sum_down(i, j)
        if highest_sum < sum:
            highest_sum = sum

for i in range(20):
    for j in range(17):
        sum = get_sum_right(i, j)
        if highest_sum < sum:
            highest_sum = sum
